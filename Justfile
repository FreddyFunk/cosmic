...
# Build Image
[group('Image')]
build $image="vivid" $tag="latest" $flavor="main" rechunk="0" ghcr="0" pipeline="0" $kernel_pin="":
    #!/usr/bin/bash
    set -eoux pipefail
    ...
    LABELS+=("--label" "io.artifacthub.package.readme-url=https://raw.githubusercontent.com/ublue-os/vivid/refs/heads/main/README.md")
...
[group('Image')]
build-rechunk image="vivid" tag="latest" flavor="main" kernel_pin="":
    @just build {{ image }} {{ tag }} {{ flavor }} 1 0 0 {{ kernel_pin }}

# Build Image with GHCR Flag
[group('Image')]
build-ghcr image="vivid" tag="latest" flavor="main" kernel_pin="":
    #!/usr/bin/bash
    if [[ "${UID}" -gt "0" ]]; then
...
[group('Image')]
build-pipeline image="vivid" tag="latest" flavor="main" kernel_pin="":
    #!/usr/bin/bash
    if [[ "${UID}" -gt "0" ]]; then
        echo "Must Run with sudo or as root..."
        exit 1
...
[group('Image')]
[private]
rechunk $image="vivid" $tag="latest" $flavor="main" ghcr="0" pipeline="0":
    #!/usr/bin/bash
    set -eoux pipefail
    ...
    just sudoif podman run --rm \
        ...
        --env LABELS="org.opencontainers.image.title=${image_name}$'\n''io.artifacthub.package.readme-url=https://raw.githubusercontent.com/ublue-os/vivid/refs/heads/main/README.md'$'\n''io.artifacthub.package.logo-url=https://avatars.githubusercontent.com/u/120078124?s=200&v=4'$'\n'" \
...
[group('Image')]
load-rechunk image="vivid" tag="latest" flavor="main":
    #!/usr/bin/bash
    set -eou pipefail
...
# Run Container
[group('Image')]
run $image="vivid" $tag="latest" $flavor="main":
    #!/usr/bin/bash
    set -eoux pipefail
...
# Build ISO
[group('ISO')]
build-iso $image="vivid" $tag="latest" $flavor="main" ghcr="0" pipeline="0":
    #!/usr/bin/bash
    set -eoux pipefail
    ...
    # Flatpaks list for vivid
    FLATPAK_DIR_SHORTNAME="vivid_flatpaks"
...
[group('ISO')]
build-iso-ghcr image="vivid" tag="latest" flavor="main":
    @just build-iso {{ image }} {{ tag }} {{ flavor }} 1

# Run ISO
[group('ISO')]
run-iso $image="vivid" $tag="latest" $flavor="main":
    #!/usr/bin/bash
    set -eoux pipefail
...
# Secureboot Check
[group('Utility')]
secureboot $image="vivid" $tag="latest" $flavor="main":
    #!/usr/bin/bash
    set -eoux pipefail
...
[group('Utility')]
[private]
fedora_version image="vivid" tag="latest" flavor="main" $kernel_pin="":
    #!/usr/bin/bash
    set -eou pipefail
    just validate {{ image }} {{ tag }} {{ flavor }}
...
[group('Utility')]
[private]
image_name image="vivid" tag="latest" flavor="main":
    #!/usr/bin/bash
    set -eou pipefail
    just validate {{ image }} {{ tag }} {{ flavor }}
    if [[ "{{ flavor }}" =~ main ]]; then
...
# Generate Tags
[group('Utility')]
generate-build-tags image="vivid" tag="latest" flavor="main" kernel_pin="" ghcr="0" $version="" github_event="" github_number="":
    #!/usr/bin/bash
    set -eou pipefail
